...
should' do
  ...
  it 'update current flag for tasks' do
    ...
    @fetcher.fetch_projects(@integration)
    @fetcher.fetch_tasks(@integration.projects.last, @integration)
    Task.find_by(source_identifier: '1').current_task.should be_true
    Task.find_by(source_identifier: '4').current_task.should be_false
    ...
    @fetcher.fetch_projects(@integration)
    @fetcher.fetch_tasks(@integration.projects.last, @integration)
    Task.find_by(source_identifier: '1').current_task.should be_false
    Task.find_by(source_identifier: '4').current_task.should be_true
